...
module tok_fees::multi_token_tests {
    ...
    fun add_sui_price(scenario: &mut test_scenario::Scenario, clock: &clock::Clock) {
        test_scenario::next_tx(scenario, ADMIN);
        let mut fee = test_scenario::take_from_sender<MultiTokenFee>(scenario);
        multi_token::add_price<SUI>(&mut fee, PRICE, clock);
    ...
    fun test_create_fee_success() {
        ...
        {
            let fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
    ...
    fun test_add_price_duplicate() {
        ...
        {
            let mut fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
            multi_token::add_price<SUI>(&mut fee, NEW_PRICE, &clock);
            test_scenario::return_to_sender(&scenario, fee);
        };
    ...
    fun test_pay_fee_success() {
        ...
        {
            let fee = test_scenario::take_from_address<MultiTokenFee>(&scenario, ADMIN);
    ...
    fun test_pay_fee_inactive() {
        ...
        {
            let mut fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
            multi_token::set_active(&mut fee, false);
            test_scenario::return_to_sender(&scenario, fee);
        };
    ...
    fun test_update_price_success() {
        ...
        test_scenario::next_tx(&mut scenario, ADMIN);
        {
            ...
            multi_token::update_price<SUI>(&mut fee, NEW_PRICE, &clock);
            assert!(multi_token::get_prices(&fee).length() == 1, 0);
            test_scenario::return_to_sender(&scenario, fee);
        };
    ...
    /// Verifies that update_price aborts if the token type does not exist in the vector.
    #[test]
    #[expected_failure(abort_code = multi_token::ETokenNotAccepted)]
    ...
    fun test_update_recipient_success() {
        ...
        test_scenario::next_tx(&mut scenario, ADMIN);
        {
            let mut fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
            clock::increment_for_testing(&mut clock, 1);
            multi_token::update_recipient(&mut fee, NEW_RECIPIENT, &clock);
    ...
    fun test_update_recipient_locked() {
        ...
        test_scenario::next_tx(&mut scenario, ADMIN);
        {
            let mut fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
            clock::increment_for_testing(&mut clock, 1);
            multi_token::update_lock_period(&mut fee, LOCK_PERIOD, &clock);
            test_scenario::return_to_sender(&scenario, fee);
        };

        // attempt update without advancing clock past new lock period — should abort
        test_scenario::next_tx(&mut scenario, ADMIN);
        {
            let mut fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
            multi_token::update_recipient(&mut fee, NEW_RECIPIENT, &clock);
    ...
    fun test_update_lock_period_locked() {
        ...
        {
            let mut fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
            multi_token::update_lock_period(&mut fee, 0, &clock);
            test_scenario::return_to_sender(&scenario, fee);
        };
    ...
    fun test_delete_fee_success() {
        ...
        test_scenario::next_tx(&mut scenario, ADMIN);
        {
            let fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
            clock::increment_for_testing(&mut clock, 1);
            multi_token::delete_fee(fee, &clock);
        };
    ...
    fun test_delete_fee_locked() {
        ...
        // attempt delete without advancing clock past new lock period — should abort
        test_scenario::next_tx(&mut scenario, ADMIN);
        {
            let fee = test_scenario::take_from_sender<MultiTokenFee>(&scenario);
            multi_token::delete_fee(fee, &clock);
